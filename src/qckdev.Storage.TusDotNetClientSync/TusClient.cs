...
namespace qckdev.Storage.TusDotNetClientSync
{
    ...
    public class TusClient
    {
        ...
        public string Create(string url, FileInfo fileInfo,
            params TusMetadata[] metadata)
        {
            if (!metadata.Any(m => m.key == "filename"))
            {
                metadata = metadata.Concat(new[] {new TusMetadata("filename", fileInfo.Name)}).ToArray();
            }

            return Create(url, fileInfo.Length, metadata);
        }
        ...
        /// <returns>The URL to the created file.</returns>
        /// <exception cref="Exception">Throws if the response doesn't contain the required information.</exception>
        public string Create(string url, long uploadLength,
            params TusMetadata[] metadata)
        {
        ...
        /// <param name="cancellationToken">A cancellation token to cancel the operation with.</param>
        /// <returns>A <see cref="TusOperation{T}"/> which represents the upload operation.</returns>
        public TusOperation<List<TusHttpResponse>> Upload(
            string url,
            Stream fileStream,
            double chunkSize = 5.0)
        {
                ...
                reportProgress =>
                {
                    try
                    {
                        ...
                        while (offset < fileStream.Length)
                        {
                            ...
                            var bytesRead = fileStream.Read(buffer, 0, uploadChunkSize);
        ...
        public TusOperation<TusHttpResponse> Download(string url) =>
